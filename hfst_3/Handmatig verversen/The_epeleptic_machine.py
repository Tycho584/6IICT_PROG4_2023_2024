...
breedte = 1920
hoogte = 1080
app.geometry(f"{breedte}x{hoogte}")  # Set a fixed window size
frame = tk.Frame(master=app, width=breedte, height=hoogte)
frame.pack()

veld = tk.Entry(master=frame)
veld.grid(row=0, column=0)

canvas = tk.Canvas(master=frame, width=breedte, height=hoogte)
...
app.protocol("WM_DELETE_WINDOW", afsluiten)
...
while True:
    ...
    label_kleur = random.choice(colors)
    veld_text = veld.get()
    canvas.config(bg=kleur, width=breedte, height=hoogte)
    veld_text = ""
    canvas.create_text(breedte // 2, hoogte // 2, text=veld_text, fill=label_kleur, font=("Arial", 50), anchor="center")
